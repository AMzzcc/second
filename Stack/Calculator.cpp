...
//		return true;
//	}
//	if (i == 0 && input[i + 1] >= '0'&&input[i + 1] <= '9')  //A在开头+num
//	{
//		//数字
//		temp.judge = true;
...
//		return true;
//	}
//	else if (i > 0 && input[i - 1] == '('&&input[i + 1] >= '0'&&input[i + 1] <= '9') //B不在开头，判断前边是否有左括号
//	{
//		//数字
//		temp.judge = true;
//		temp.num = input[i + 1] - '0';
...
//			return true;
//		}
//		else if (input[i] == '+' || input[i] == '-') //2.括号内负号作前缀，且后面没跟左括号
//		{
//			temp.num = -temp.num;
//			q.push(temp);//操作数进入后缀表达式
//			return true;
...
//			while (true)
//			{
//				if (i < (int)input.length() && input[i] >= '0'&&input[i] <= '9')
//				{
//					temp.num = input[i] - '0';
//					i++;
//					while (i < (int)input.length() && input[i] >= '0'&&input[i] <= '9')
...
//					temp.judge = false;
//					temp.oper = op;
//					while (!s.isEmptyStack() && compare(s.top->next->data.oper, temp.oper))
//					{
//						q.push(s.top->next->data);
//						s.pop();
//					}
